...
The admins of the repo can publish the book after it has been edited. Detailed instructions can be found [here](https://quarto.org/docs/books/), but the short version is: **"Use the `quarto render` command from the Terminal to render all output formats; the output of your book will be written to the `_book` sub-directory of your book project.**:
...
Finally, you need to publish the book. The following will automatically publish to online with Github Pages. 
